...
const CalendarItem: FC<{
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        aria-label={t('recycle_post', 'Recycler la publication')}
        data-tooltip-id="tooltip"
        data-tooltip-content={t(
          'recycle_post_help',
          'Créer une nouvelle publication à partir de ce contenu'
        )}
        onClick={(event) => {
        ...
        className={clsx(
          'absolute right-[5px] top-[29px] z-20 rounded-[5px] bg-btnPrimary px-[7px] py-[4px] text-[10px] font-semibold text-white shadow transition-opacity',
          isImported ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
        )}
      >
        {t('recycle', 'Recycler')}
      </button>
